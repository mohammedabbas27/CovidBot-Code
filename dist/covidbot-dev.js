...
function main() {
  ...
  $.getScript("https://cdnjs.cloudflare.com/ajax/libs/dompurify/2.2.6/purify.min.js", function () {
    ...
    function setUserResponse(val) {
      ...
      noUserInput = false;
      var BotResponse = "<div class=\"chatbot-bot-msg\" tabindex=0 aria-label=\"loading for bot response\" id=\"typing\" style=\"font-size:".concat(fontSize, "px;border:0px !important;background:transparent !important;box-shadow:none !important\">") + "<div id=\"chatbot-loading-msg\">\n                      <span class=\"chatbot-loading-dot\"></span>\n                      <span class=\"chatbot-loading-dot\"></span>\n                      <span class=\"chatbot-loading-dot\"></span>\n                      </div>" + '</div><div class="chatbot-clearfix"></div>';
      $(DOMPurify.sanitize(BotResponse)).appendTo(".chatbot-msgs").hide().fadeIn(100);
    ...
    function setBotResponse(val) {
      ...
      setTimeout(function () {
        if (val.length < 1) {
          ...
          msg = "Sorry, the chatbot is offline. Please try again later.";
          var BotResponse = "<p class=\"chatbot-bot-msg\" style=\"font-size:".concat(fontSize, "px;\" tabindex=0>") + msg + '</p><div class="chatbot-clearfix"></div>';
          $(DOMPurify.sanitize(BotResponse)).appendTo(".chatbot-msgs").hide().fadeIn(200);
        } else {
          ...
          for (var i = 0; i < val.length; i++) {
            ...
            if (val[i].hasOwnProperty("custom")) {
              ...
              if (type == "buttons") {
                ...
                var BotResponse = "<p class=\"chatbot-bot-msg\" style=\"font-size:".concat(fontSize, "px;\" tabindex=0>") + data.text + '</p><div class="chatbot-clearfix"></div>';
                $(DOMPurify.sanitize(BotResponse)).appendTo(".chatbot-msgs").hide().fadeIn(200);
                ...
                markup += "</ul>";
                var BotResponse = "<div class=\"chatbot-bot-msg style=\"font-size:".concat(fontSize, "px;\" overflow-scroll\" tabindex=0>") + markup + '</div><div class="chatbot-clearfix"></div>';
                $(DOMPurify.sanitize(BotResponse)).appendTo(".chatbot-msgs").hide().fadeIn(200); //   addSuggestion(data.buttons, "chatbot-buttons-menu");
              }
            }
            ...
            if (val[i].hasOwnProperty("text")) {
              ...
              if (val[i].text.indexOf("https") >= 0) {
                ...
                var BotResponse = "<p class=\"chatbot-bot-msg\" style=\"font-size:".concat(fontSize, "px;\" tabindex=0>") + val[i].text + '</p><div class="chatbot-clearfix"></div>';
                $(DOMPurify.sanitize(BotResponse)).appendTo(".chatbot-msgs").hide().fadeIn(200);
              }
