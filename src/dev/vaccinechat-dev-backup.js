...
function main() {
  ...
  $.getScript(
    ...
    function () {
              ...
              !isChatbotIconMini ? "" : "chatbot-hide-elem"
                ...
                <div class="chatbot-widget" id="chatbot-widget">
                  <div class="chatbot-header">
                    ...
                    <div aria-hidden="true" id="suggestion-info" class="visuallyHidden">
                        select one of the suggestion listed below, use TAB and SHIFT + TAB. You can leave the list with ESC anytime.
                    </div>
          ...
          .limit-reached{
           ...
           .chatbot-keypad *:focus, .chatbot-shortcuts-info *:focus, .chatbot-inner-bubble *:focus {
            ...
            .suggestion-dropdown-button{
              width: 80%;
              background: #ffffff;
              box-shadow: 0px 0px 5px 1px #e3e3e7;
              border: 1px solid #ffffff;
              padding: 0.6em 1em;
              border-radius: 2px;
              display: flex;
            ...
            .suggestion-select-item:focus{
              background: #388557;
              color: white;
              border-color: #388557 !important;
            }
  
            .suggestion-select-item:last-child{
              border:none;
            }
      ...
      $(document).on("keydown", ".suggestion-select-item", function (e) {
      ...
      function addSuggestion(textToAdd, type) {
        setTimeout(function () {
          ...
          var markup = `<div class="suggestion-container"><button id="suggestion-dropdown-button" class="suggestion-dropdown-button" aria-label="Select an option from the list"><span>Select an option</span><span class="material-icons">
            keyboard_arrow_down
            </span></button>
            <div class="suggestion-dropdown"><ul role="menu" style="list-style:none" tabindex="0"> `;
          for (let i = 0; i < suggLength; i++) {
            markup += `<li style="font-size:${fontSize}px" class="suggestion-select-item ${
            ...
            markup +=
              ...
              "'>" +
              `<div role="menuitem" aria-selected="false">` +
              suggestions[i].title +
              "</div></li>";
          }
          markup += `</ul></div></div>`;
      ...
      $("#chatbot-widget").keydown(function (e) {
        if (e.keyCode == 27) {
          ...
          if ($(".suggestion-dropdown").hasClass("open")) {
            $(".suggestion-dropdown").removeClass("open");
            return;
          }
          hideChatBot();
        }
      ...
      function toggleSuggestionDropdown(ele) {
        $(ele).parent().find(".suggestion-dropdown").toggleClass("open");
        scrollToBottomOfResults();
        setTimeout(() => {
          $(".suggestion-dropdown ul").children().first().focus();
        }, 500);
      }

      /* Toggle suggestion dropdown */

      $(document).on("click", ".suggestion-dropdown-button", function (e) {
        toggleSuggestionDropdown(this);
      });

      $(document).on("keydown", ".suggestion-dropdown-button", function (e) {
        if (e.keyCode == 40) {
          toggleSuggestionDropdown(this);
        }
      });
      ...
      $(document).on("click keypress", ".suggestion-select-item", function (e) {
        if (e.type === "keypress") {
          if (!(e.keyCode != 13 || e.keyCode != 32)) {
            return;
          }
        ...
        $(this).closest(".suggestion-dropdown").removeClass("open");
      });
